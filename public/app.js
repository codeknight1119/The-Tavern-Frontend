...
document.getElementById("userSearchBttn").addEventListener("click", async () => {
    ...
    try {
        ...
        for (const val of docs) {
            ...
            const addRoleBtn =
                searchedRes.querySelector(".searched-addRole-btn");

            const revokeRoleBtn =
                searchedRes.querySelector(".searched-revokeRole-btn");
            ...
            if (revokeRoleBtn) {
                ...
                revokeRoleBtn.addEventListener(
                    "click",
                    () => {
            ...
            const saveBtn =
                searchedRes.querySelector(".searched-save");
...
function startResizing(event) {
    ...
    // Define a minimum width (e.g., 100px) and a maximum width (e.g., container - 100px)
    const minWidth = 100;
    const maxWidth = maxContainerWidth - 100;

    // Constrain the new width
    newWidth = Math.max(minWidth, Math.min(newWidth, maxWidth));

    // Apply the new width to the left side and divider
...
async function fetchServer(type, data) {
    const url = "https://the-tavern-backend.onrender.com/" + type;
    let options = {
        method: "POST",
        headers: {
...
// Prevent the popup from showing again in this tab once the server has woken up
let serverIsReady = false;

async function checkServerStatus() {
    if (!loadingPopup || serverIsReady) return;

    loadingPopup.style.display = "block";

    try {
        const result = await fetchServer("serverStatus");
