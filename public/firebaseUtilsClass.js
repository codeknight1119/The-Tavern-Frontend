...
export class Firebase {
    ...
    async getDocuments(path, l, docParam, arrayFilter) {
        try {
            const constraints = [];
            if (arrayFilter?.field && arrayFilter.value !== undefined) constraints.push(where(arrayFilter.field, Array.isArray(arrayFilter.value) ? 'array-contains-any' : '==', arrayFilter.value));
            if (docParam?.field) constraints.push(orderBy(docParam.field, docParam.direction || 'asc'));
            if (typeof l === 'number' && l > 0) constraints.push(limit(l));
            return (await getDocs(query(collection(this.db, path), ...constraints))).docs.map(d => ({ id: d.id, ...d.data() }));
        } catch (e) { console.log(e); throw e; }
    }
    async getDocumentFieldIncludes(path, field, text) {
        try { const snap = await getDocs(query(collection(this.db, path), where(field, '>=', text), where(field, '<=', text + '\uf8ff'))); return snap.docs.map(d => ({ id: d.id, ...d.data() })); }
        catch (e) { console.log(e); throw e; }
    }
    listenForNewDocInCollection(path, callback) { return onSnapshot(query(collection(this.db, path), where('timestamp', '>', Date.now())), snap => snap.docChanges().forEach(change => { if (change.type === 'added') callback(change.doc.data()); })); }
    async deleteDocument(path) { try { return await deleteDoc(doc(this.db, path)); } catch (e) { console.error(e); throw e; } }
    isSignedIn() { return new Promise((resolve, reject) => { try { const unsubscribe = this.auth.onAuthStateChanged(user => { unsubscribe(); resolve(user ? { user, isNew: user.metadata.creationTime === user.metadata.lastSignInTime } : null); }, reject); } catch (e) { console.error('error + ' + JSON.stringify(e)); reject(e); } }); }
